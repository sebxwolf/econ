...
**Mean Prices for Hellman:**
```{r Task 2a, echo=F, warning=F, results='asis'}
...
stargazer(df_sum1,df_sum2, header=F)
```
**Mean Prices for Kraft:**
```{r Task 2b, echo=F, warning=F, fig.height = 2}
...
stargazer(simple_hellman_jewel,simple_kraft_jewel,simple_hellman_central,simple_kraft_central,column.labels=c("Hellman in Chicago","Kraft in Chicago","Hellman in Midwest","Kraft in Midwest"),df=F,header=FALSE)
```
...
```{r Task D4}
#Hellman unit change
print(1.1^(lm(ln_sales_u~ln_price,hellman_at_jewel)[["coefficients"]][["ln_price"]])-1)
#Kraft unit change
print(1.1^(lm(ln_sales_u~ln_price,kraft_at_jewel)[["coefficients"]][["ln_price"]])-1)
```
***
...
lm(ln_sales_u_hellman_at_jewel~ln_price_hellman_at_jewel+ln_price_kraft_at_jewel,x)
...
**Answer:**
Kraft would have to lower their price by 3.64% in order to keep sales constant in Chicago given a 10% price decrease by Hellman, assuming our model is correctly specified.
```{r Task D8}
print(1-1/((0.9^
      (lm(ln_sales_u_kraft_at_jewel~ln_price_kraft_at_jewel+ln_price_hellman_at_jewel,x)
